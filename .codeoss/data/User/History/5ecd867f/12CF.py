...
class FraudDetectionService:
    def __init__(self):
        ...
        # Set up Gemini API (load from environment variable)
        self.api_key = os.getenv('GEMINI_API_KEY')
        if not self.api_key:
            raise ValueError("GEMINI_API_KEY not set in environment variables. Please add it to your .env file.")
        try:
            genai.configure(api_key=self.api_key)
            self.gemini_model = genai.GenerativeModel('gemini-1.5-flash')  # Or 'gemini-1.0-pro'
            self.embedding_model = 'models/embedding-001'  # Gemini embedding model
            # Predefined fraud patterns (embeddings; use realistic 768-dim dummies)
            self.fraud_patterns = np.random.rand(2, 768)  # Replace with real 768-dim embeddings from known fraud data
            self.gemini_available = True
            print("✅ Gemini API initialized successfully")
        except Exception as e:
            print(f"❌ Gemini API initialization error: {e}. Falling back to local methods.")
    ...
    def get_sentiment(self, text):
        """Extract sentiment polarity and subjectivity from text (fallback to TextBlob if Gemini unavailable)"""
        if not text or pd.isna(text):
            return 0.0, 0.0
        
        if self.gemini_available:
            try:
                response = self._gemini_predict(text, task='sentiment')
    ...
    def _gemini_predict(self, text, task):
        """Helper to call Gemini API for specific tasks and return full details"""
        details = {'task': task, 'prompt': '', 'raw_response': ''}
        
        if task == 'sentiment':
            details['prompt'] = f"Analyze sentiment of this job posting. Respond ONLY with valid JSON (no extra text, no backticks): {{'sentiment': polarity (-1 to 1), 'subjectivity': (0 to 1)}}: {text}"
            response = self.gemini_model.generate_content(details['prompt'])
            details['raw_response'] = response.text.strip()
            # Clean and parse JSON
            cleaned = re.sub(r'``````', '', details['raw_response']).strip()
            cleaned = cleaned.replace("'", '"')  # Replace single quotes with double
            try:
                parsed = json.loads(cleaned)
            except json.JSONDecodeError as e:
                print(f"JSON parse error in sentiment: {e}. Raw: {details['raw_response']}")
                parsed = {'sentiment': 0.0, 'subjectivity': 0.0}  # Fallback values
            return {'parsed': parsed, 'details': details}
        
        elif task == 'classify_ai_generation':
            details['prompt'] = f"Is this text likely AI-generated? Respond ONLY with a number: probability (0-1): {text}"
            response = self.gemini_model.generate_content(details['prompt'])
            details['raw_response'] = response.text.strip()
            # Clean and parse float
            cleaned = re.sub(r'``````', '', details['raw_response']).strip()
            try:
                parsed = {'ai_prob': float(cleaned)}
            except ValueError as e:
                print(f"Float parse error in ai_generation: {e}. Raw: {details['raw_response']}")
                parsed = {'ai_prob': 0.0}  # Fallback
            return {'parsed': parsed, 'details': details}
        
        elif task == 'embedding':
            details['prompt'] = text  # Embeddings don't use a prompt, but store input
            response = genai.embed_content(
                model=self.embedding_model,
                content=text,
                task_type="SEMANTIC_SIMILARITY"
            )
            details['raw_response'] = str(response['embedding'])  # Store as string for simplicity
            parsed = {'embedding': response['embedding']}
            return {'parsed': parsed, 'details': details}
        
        elif task == 'recommend_genuine':  # New task for direct recommendation
            details['prompt'] = f"Analyze this job posting and recommend if it is genuine or fraudulent. Respond ONLY with: 'Genuine' or 'Fraudulent', followed by a short reason: {text}"
            response = self.gemini_model.generate_content(details['prompt'])
            details['raw_response'] = response.text.strip()
            # Simple parse: first word is recommendation, rest is reason
            parts = details['raw_response'].split(' ', 1)
            parsed = {'recommendation': parts[0] if parts else 'Unknown', 'reason': parts[1] if len(parts) > 1 else ''}
            return {'parsed': parsed, 'details': details}
        
        return {'parsed': {}, 'details': details}
    
    def predict_fraud(self, job_data):
        """Main prediction function with Gemini API integration and complex calculations"""
        try:
            # Extract features for analysis
            features = self.extract_features(job_data)
            
            # Prepare combined text for Gemini
            combined_text = ' '.join([
                job_data.get('title', ''),
                job_data.get('description', ''),
                job_data.get('requirements', ''),
            ...
            # Step 2: Gemini enhancements (if available) - Now capturing details
            sentiment_score = features['desc_sentiment']  # Default from local
            ai_generation_score = 0.0
            embedding_similarity = 0.0
            ai_analysis_details = {}  # To store all AI task details
            
            if self.gemini_available and cleaned_text:
                try:
                    # Sentiment
                    sentiment_response = self._gemini_predict(cleaned_text, task='sentiment')
                    ai_analysis_details['sentiment'] = sentiment_response['details']
                    sentiment_score = sentiment_response['parsed'].get('sentiment', sentiment_score)  # -1 to 1
                    
                    # AI-generated content detection
                    ai_gen_response = self._gemini_predict(cleaned_text, task='classify_ai_generation')
                    ai_analysis_details['ai_generation'] = ai_gen_response['details']
                    ai_generation_score = ai_gen_response['parsed'].get('ai_prob', 0.0)  # 0-1
                    
                    # Embeddings similarity to fraud patterns
                    embedding_response = self._gemini_predict(cleaned_text, task='embedding')
                    ai_analysis_details['embedding'] = embedding_response['details']
                    embedding = np.array(embedding_response['parsed'].get('embedding', [0.0] * 768))  # Assuming 768-dim
                    ...
                    # New: Direct recommendation from Gemini
                    recommend_response = self._gemini_predict(cleaned_text, task='recommend_genuine')
                    ai_analysis_details['recommendation'] = recommend_response['details']
                except Exception as e:
                    print(f"Gemini API error: {e}. Using local fallbacks.")
                    ai_analysis_details['error'] = str(e)
            ...
            # Return comprehensive result with AI details
            result = {
                ...
                'method': f"{method} + Gemini API" if self.gemini_available else method,
                'features': features,  # Include features for debugging
                'ai_generation_score': ai_generation_score,
                'embedding_similarity': embedding_similarity,
                'ai_analysis_details': ai_analysis_details  # New: Stored for database and display
            }
