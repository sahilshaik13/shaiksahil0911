...
class FraudDetectionService:
    def __init__(self):
        ...
        self.api_key = 'AIzaSyCg_bQKGhoE3WJyUvWv9-t3eftKO4UBOS4'  # Get from https://aistudio.google.com/app/apikey
        try:
    ...
    def _gemini_predict(self, text, task):
        ...
        if task == 'sentiment':
            details['prompt'] = f"Analyze sentiment of this job posting. Return as JSON: {{'sentiment': polarity (-1 to 1), 'subjectivity': (0 to 1)}}: {text}"
            response = self.gemini_model.generate_content(details['prompt'])
            details['raw_response'] = response.text.strip()
            # Parse JSON from response (assumes clean output; add error handling as needed)
            import json
            parsed = json.loads(details['raw_response'])
            return {'parsed': parsed, 'details': details}
        
        elif task == 'classify_ai_generation':
            details['prompt'] = f"Is this text likely AI-generated? Return only a number: probability (0-1): {text}"
            response = self.gemini_model.generate_content(details['prompt'])
            details['raw_response'] = response.text.strip()
            return {'parsed': {'ai_prob': float(details['raw_response'])}, 'details': details}
            ...
            return {'parsed': {'embedding': response['embedding']}, 'details': details}
