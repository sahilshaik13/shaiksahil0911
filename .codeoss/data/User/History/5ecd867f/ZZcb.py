...
class FraudDetectionService:
    def __init__(self):
        ...
        self.api_key = 'AIzaSyBbm3x2VY_CKwgCR0lXssR9LhG2BrWy2QA'  # Get from https://aistudio.google.com/app/apikey
        try:
    ...
    def _gemini_predict(self, text, task):
        """Helper to call Gemini API for specific tasks"""
        if task == 'sentiment':
            prompt = f"Analyze sentiment of this job posting. Return as JSON: {{'sentiment': polarity (-1 to 1), 'subjectivity': (0 to 1)}}: {text}"
            ...
            prompt = f"Is this text likely AI-generated? Return only a number: probability (0-1): {text}"
            response = self.gemini_model.generate_content(prompt)
            return {'ai_prob': float(response.text.strip())}
    ...
    def predict_fraud(self, job_data):
        ...
        try:
            ...
            # Step 2: Gemini enhancements (if available)
            ...
            if self.gemini_available and cleaned_text:
                try:
                    # Sentiment (override local if Gemini succeeds)
