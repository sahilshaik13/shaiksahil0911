...
from django.db.models import Count, Q, Avg  # Added Avg here
import json
...
def predict_fraud(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Save prediction with new fields, including AI details
            fraud_prediction = FraudPrediction.objects.create(
                ...
                ai_generation_score=prediction['ai_generation_score'],
                embedding_similarity=prediction['embedding_similarity'],
                ai_analysis_details=prediction.get('ai_analysis_details', {})  # New: Save AI analysis details
            )
            ...
            if request.content_type == 'application/json':
                return JsonResponse({
                    ...
                    'prediction': {
                        ...
                        'risk_level': prediction['risk_level'],
                        'ai_generation_score': f"{prediction['ai_generation_score']:.1%}",
                        'embedding_similarity': f"{prediction['embedding_similarity']:.1%}"
                    },
            ...
            error_msg = str(e)
            if 'Gemini API' in error_msg:  # Updated to Gemini (from Vertex AI)
                error_msg = f"Gemini API error: {error_msg}. Check API key."
            if request.content_type == 'application/json':
                return JsonResponse({'success': False, 'error': error_msg})
            else:
                return render(request, 'fraud_detection/error.html', {'error': error_msg})
...
def analytics_dashboard(request):
    ...
    # Industry analysis (updated to include new metrics if needed)
    industry_stats = JobPosting.objects.exclude(industry='').values('industry').annotate(
        ...
        avg_ai_score=Avg('fraud_prediction__ai_generation_score')  # Changed to Avg (imported above)
...
def job_detail(request, job_id):
    """View individual job posting details with AI analysis visibility"""
    try:
        ...
        context = {
            'job_posting': job_posting,
            'fraud_prediction': fraud_prediction,
            'ai_analysis_details': fraud_prediction.ai_analysis_details  # New: Pass to template for display
        }
