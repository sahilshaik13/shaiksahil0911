...
class FraudPrediction(models.Model):
    ...
    # AI-specific metrics (added for Vertex AI integration)
    ai_generation_score = models.FloatField(
        null=True,
        blank=True,
        validators=[MinValueValidator(0.0), MaxValueValidator(1.0)],
        help_text="Probability that the content is AI-generated (0-1)"
    )
    embedding_similarity = models.FloatField(
        null=True,
        blank=True,
        validators=[MinValueValidator(0.0), MaxValueValidator(1.0)],
        help_text="Similarity to known fraud patterns based on embeddings (0-1)"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['ai_generation_score']),  # New index for querying AI metrics
            models.Index(fields=['embedding_similarity']),  # New index for querying embeddings
        ]
